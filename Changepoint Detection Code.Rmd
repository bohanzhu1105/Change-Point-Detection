...
\vspace{5pt}
In our changepoint model, the large Raftery-Lewis diagnostic value is mainly 
due to strong autocorrelation between samples. Since the changepoint is a discrete 
parameter with a naturally low acceptance rate, the Markov chain tends to stay at 
the same value for many iterations. This stickiness leads to high correlation 
between consecutive samples, increasing the number of effective samples needed 
to estimate the target quantile accurately. As a result, the Raftery-Lewis statistic 
becomes large, which is expected and reasonable in changepoint detection problems.
